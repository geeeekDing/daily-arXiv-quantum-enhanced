...
TL;DR: 本文从参数依赖常微分方程角度研究Jost函数的解析性质，通过因子化动量相关的分支项，将散射问题转化为系数为复能量单值解析函数的微分系统，证明了变换后的Jost函数在有限径向距离上是复能量的单值解析函数，并讨论了因子化过程的几何解释。


<details>
  <summary>Details</summary>
Motivation: Jost函数的解析性质是量子散射理论和散射矩阵复能量平面解析延拓的基础，但能量变量与动量之间的平方根关系导致多值性，需要从常微分方程参数依赖理论的角度澄清其单值解析性的根源。

Method: 从短程中心势的径向薛定谔方程出发，推导Riccati-Bessel和Riccati-Neumann解的系数函数的一阶微分系统；显式因子化动量相关的分支项，将散射问题转化为系数为复能量单值解析函数的微分系统；利用常微分方程解对参数的解析依赖经典理论证明变换后Jost函数的解析性。

Result: 变换后的Jost函数在有限径向距离上是复能量的单值解析函数。

Conclusion: 本文从常微分方程参数依赖角度阐明了Jost函数的解析性质，通过因子化分支项解决了能量-动量平方根关系导致的多值性问题，并给出了因子化过程的黎曼曲面拓扑解释。
...
TL;DR: 本文发展了一个用于分析受反谐振子势约束的Klein-Gordon标量场的量子与热性质的系统框架。通过非厄米动量替换和辛相空间旋转，将系统映射到可解析处理的谐振子，从而导出配分函数、自由能、熵和热关联函数的闭式表达式，并应用于宇宙暴胀、黑洞视界和凝聚态相变三个物理场景。


<details>
  <summary>Details</summary>
Motivation: 现有文献中关于反谐振子势下标量场的结果分散且缺乏统一框架，需要一种系统方法处理该不稳定量子系统的量子与热力学性质，特别是有限温度下的谱密度和纠缠熵。

Method: 采用非厄米动量替换$P \to P - mωx$，再通过辛相空间旋转$V = \exp[-\frac{\pi}{8}(xp+px)]$将系统映射到有效谐振子（在$x e^{iπ/4}$处评估），从而定义正则化的配分函数并导出闭式表达式。

Result: 得到了自由能、熵和热关联函数的闭式解析表达式，并统一了文献中分散的结果，给出了不稳定量子系统有限温度谱密度和纠缠熵的新预测。

Conclusion: 该框架可统一应用于宇宙暴胀时期标量场涨落、黑洞视界附近量子场以及凝聚态二级相变附近的序参量动力学三个物理场景，为不稳定量子系统提供了系统的热力学描述。
...
TL;DR: 本文以固体球在重力作用下沿斜面滚动（无滑动）为物理范例，详细回顾并分析了含约束哈密顿系统的狄拉克量子化方法，重点展示了狄拉克-伯格曼算法及狄拉克括号到对易子的替换过程，并证明将约束系统的哈密顿算符限制到物理希尔伯特子空间可复现内禀方法得到的薛定谔方程，验证了狄拉克量子化方法的一致性。


<details>
  <summary>Details</summary>
Motivation: 通过一个同时包含完整约束、非完整约束以及规范系统的简单物理实例（球体沿斜面滚动），系统性地展示并验证狄拉克量子化方法在含约束哈密顿系统中的应用一致性。

Method: 以球体沿斜面滚动（无滑动）为引导性范例，运用狄拉克-伯格曼算法处理完整与非完整约束，将经典狄拉克括号替换为量子对易子，并通过对约束进行量子化识别物理希尔伯特子空间。

Result: 证明了将约束系统的哈密顿算符限制到物理希尔伯特子空间后，所得到的薛定谔方程与内禀方法（intrinsic terms）直接导出的方程完全一致。

Conclusion: 该结果强化了狄拉克量子化方法在处理含约束系统（包括规范系统）时的内部一致性与有效性。
...
TL;DR: 本文系统研究了极保真度、Holevo基和广义保真度的酉因子。证明了单边极保真度及其对称化在(-∞,1]上不减、在[1,∞)上不增，极坐标路径实现了[-1,1]区间上的所有保真度值（z≥1/2以及对数欧几里得保真度），但对于d≥2且0<z<1/2则一般失败。还求解了固定对的Holevo基方程，分类了所有Holevo基，并精确刻画了哪些酉因子可由基R产生。


<details>
  <summary>Details</summary>
Motivation: 解决Afham-Ferrie广义保真度理论中的若干开放问题，深入理解极保真度、Holevo基以及广义保真度中的酉因子结构。

Method: 利用复正定矩阵锥上的极分解与极坐标路径，通过单调性分析和算子理论方法研究保真度函数的性质，并对Holevo基方程进行代数分类。

Result: (1) 极保真度在(-∞,1]单调递增、[1,∞)单调递减；(2) 极坐标路径实现z≥1/2和对数欧几里得保真度，但0<z<1/2失败；(3) 解出了Holevo基方程并分类了所有Holevo基；(4) 酉因子可产生的充要条件是P^{-1/2}Q^{1/2}W相似于正定矩阵，否定了全局反向包含SU(d)猜想。

Conclusion: 极保真度理论提供了广义保真度的统一框架，Holevo基的分类和酉因子的刻画完善了Afham-Ferrie理论，并推翻了一些先前猜测的包含关系。
...
TL;DR: 本文研究了通过玻色子环境相互作用的两自旋系统的时间演化，分析了不同环境模型下纠缠度（ negativity）的变化，获得了最大化纠缠同时最小化解相干的环境参数条件，并推导了有效酉算子。


<details>
  <summary>Details</summary>
Motivation: 量子计算和量子信息的快速发展需要有效实现量子计算的物理模型，而环境作为模型的重要组成部分，既导致退相干又产生自旋间相互作用从而诱导纠缠，因此研究环境对量子系统行为的影响对实现量子信息器件至关重要。

Method: 考虑两个自旋通过玻色子环境介导的相互作用，研究其时间演化；以negativity作为纠缠度量，对不同环境模型下的纠缠行为进行分析。

Result: 获得了实现自旋间最大纠缠值的环境参数条件；找到了能够最小化解相干同时最大化纠缠的环境模型；由于退相干影响极小，推导出了描述相应演化的有效酉算子。

Conclusion: 通过适当选择环境参数和模型，可以在抑制退相干的同时最大化自旋间的纠缠，并且当退相干足够小时，系统演化可由有效酉算子精确描述，这对量子信息器件的实现具有指导意义。
...
TL;DR: 本文推导了位置空间与波矢（动量）空间中方差受限制的尖锐不确定性关系，在经典光束理论、量子相干光束理论和单光子量子理论中均具有相同形式 ΔrΔk ≥ 5/2。


<details>
  <summary>Details</summary>
Motivation: 旨在推导出比传统海森堡不确定性关系更严格的（尖锐的）不确定性关系，以更精确地限制位置空间和波矢（动量）空间中方差的乘积下界。

Method: 通过理论推导，分别在经典光束理论、量子相干光束理论以及单光子量子理论三个框架下，导出位置-波矢不确定性关系的具体形式。

Result: 在所有三种理论框架下，不确定性关系均统一为 ΔrΔk ≥ 5/2，表明该界限具有普适性。

Conclusion: 该尖锐不确定性关系为经典与量子光学中的空间-动量约束提供了统一且更紧的极限，具有重要的理论基础意义。
...
TL;DR: 本文提出了一种在无穷体积极限下严格定义强对称性的局部框架，揭示了强到弱对称性破缺的局域本质，并引入了介于强对称与弱对称之间的冯·诺伊曼对称性，导出了适用于量子自旋链的Lieb-Schultz-Mattis型 anomalies 约束。


<details>
  <summary>Details</summary>
Motivation: 先前关于强到弱对称性破缺的描述被认为是一种相当非局域效应，缺乏局域化的理解与诊断手段。

Method: 在无穷体积极限下给出强对称性的严格定义，推广传统有限体积定义；提出若干等价表述，包括"局域电荷相干性"概念；引入冯·诺伊曼系统作为中间对称类型。

Result: 建立了强对称性的局域化诊断框架，提出了冯·诺伊曼对称性这一新概念，并导出了针对冯·诺伊曼对称性（特别地，强对称性）的Lieb-Schultz-Mattis型 anomalies 约束。

Conclusion: 强到弱对称性破缺可以以显式局域的方式理解和诊断，冯·诺伊曼对称性在无穷体积对称性中填补了强对称与弱对称之间的空白。
...
TL;DR: 本文研究量子拓扑数据分析中，哪些量子编码方式能最好地保留原始经典数据集的拓扑不变量，从而允许直接对经典数据应用量子算法提取拓扑特征。


<details>
  <summary>Details</summary>
Motivation: 传统量子TDA通常从组合对象（如单纯复形）出发，而非直接处理经典数据集，且构造这些组合对象资源消耗大。本文旨在探索更直接的路径：直接对经典数据进行量子编码，并研究哪些编码方式能保持原始数据的拓扑不变量。

Method: 分析了不同量子编码方式对经典数据拓扑不变量的保持性，重点关注直接作用于数据的编码方案，而非传统的基于单纯复形的间接方法。

Result: 确定了哪些量子编码对于直接应用量子算法提取经典数据集的拓扑特征是允许的（即能保持拓扑不变量）。

Conclusion: 为量子TDA提供了一种更直接的范式，即从原始数据直接编码而非从组合对象出发，并给出了编码方案所需满足的条件。
...
TL;DR: 本文针对硅自旋量子比特平台上的高保真逻辑魔法态生成进行了资源分析，比较了多种架构和纠错码，结合自底向上与自顶向下的方法论，发现优化控制脉冲可减少42%的魔法态蒸馏开销，且硅定制的偏置纠错码相比表面码能实现约三倍的物理占用降低。


<details>
  <summary>Details</summary>
Motivation: 在硅自旋量子比特平台上实现容错量子计算，需要高效生成高保真的逻辑魔法态，但目前缺乏系统性的资源分析与架构比较，尤其缺乏结合硬件级噪声模型与系统级应用需求的综合评估框架。

Method: 采用自底向上与自顶向下相结合的方法：自底向上基于硅处理器哈密顿量构建硬件级噪声模型（含1/f非马尔可夫噪声），评估达到目标逻辑错误率所需的物理资源；自顶向下固定目标逻辑保真度，推导对硬件性能的约束。比较了穿梭式SpinBus、密集最近邻、混合穿梭连接补丁三种架构，以及表面码、颜色码和偏置纠错码，分析了5→1和15→1魔法态蒸馏协议。

Result: 优化控制脉冲使魔法态蒸馏开销降低42%；硅定制的偏置纠错码在无需物理偏置保持操作的情况下，相比表面码实现约三倍的物理占用缩减。

Conclusion: 该框架能够系统性地评估资源缩减策略，为硅自旋量子比特平台上的容错量子计算架构设计提供了定量指导。
...
TL;DR: 本文提出在量子设备上为一组重要衍生品定价的方法，相比经典方法具有优势，结合另一项工作，在某些路径依赖期权定价中相比最先进的量子蒙特卡洛方法实现了指数级量子比特需求优势。


<details>
  <summary>Details</summary>
Motivation: 现有量子方法在定价某些路径依赖期权时量子比特需求过高，限制了其实际应用；本文旨在开发更高效的量子定价方法。

Method: 开发了必要的量子定价方法，并与另一项工作（GumaroS2026）相结合。

Result: 相比现有经典方法具有优势；在定价某些具有路径依赖收益的期权合约时，相比最先进的量子蒙特卡洛方法，实现了指数级的量子比特需求减少。

Conclusion: 所提出的方法在量子设备上为衍生品定价提供了有效方案，特别是在量子比特效率方面取得了显著突破。
...
TL;DR: 本文在IBM量子处理器上实现了一种基于纠缠动力学的质数识别量子协议，通过提取二分量子系统中纠缠演化的傅里叶分量来判断整数的素性，并引入全局缩放因子进行噪声抑制，理论部分给出了新的傅里叶模解析界，增强了对质数与合数的区分能力。


<details>
  <summary>Details</summary>
Motivation: 探索在含噪中等规模量子（NISQ）设备上实现数论应用的可能性，特别是利用量子纠缠动力学的特性来识别质数，克服实验噪声对量子计算的限制。

Method: 1) 将整数的素性与二分量子系统纠缠演化的特定傅里叶分量相关联；2) 在IBM量子处理器上实现该量子协议；3) 引入基于全局缩放因子的噪声抑制方法，在部分电路上校准后外推到不同配置；4) 推导了初始均匀叠加态下傅里叶模的新解析界。

Result: 新的解析界在中等实验偏差下增强了质数与合数之间的分离效果；噪声抑制方法有效缓解了实验噪声的影响；在NISQ设备上成功实现了质数识别。

Conclusion: 该工作向着在NISQ设备上实现实用的数论应用迈出了一步，展示了利用量子纠缠动力学进行质数识别的可行性，且噪声抑制策略具有实用价值。
...
TL;DR: 本文提出了强到弱自发对称性破缺（SW-SSB）的局域化定义，基于局域单点保真度关联子，相比传统的两点保真度关联子定义，具有更易在大系统中检测、可在热力学极限下良好定义等优势，并证明了其关键性质在局域框架下依然成立。


<details>
  <summary>Details</summary>
Motivation: 克服传统两点保真度关联子定义在热力学极限下密度矩阵无法定义的问题，并提高在大系统中的可检测性，使SW-SSB序的探测更高效。

Method: 提出局域单点保真度关联子替代传统两点保真度关联子来定义SW-SSB；通过有限深度对称信道的稳定性分析和长程条件互信息的计算验证其性质；利用共形场论基态、弹道及扩散自由费米子金属等临界系统推导普适标度行为。

Result: 局域保真度序可在系统尺寸N下用poly(N)资源探测到体积尺度O(log(N))；局域SW-SSB序在热力学极限下仍良好定义；有限深度对称信道下的稳定性和长程条件互信息等关键性质得以保持；在临界系统中导出了局域保真度关联子的普适标度行为。

Conclusion: 局域化定义在概念上与局域热化（如ETH纯态）类比，为SW-SSB提供了一个更实用且理论上自洽的框架，并定义了临界系统中一类有趣的缺陷问题。
...
TL;DR: 本文提出利用时空纠缠光子对和符合探测后选择空间相关事件的方法，从弱散射介质中分离弹道光子，提高成像对比度，并通过数值模拟和实验验证了该方法在两种配置下的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统通过时间门控或空间滤波提高散射介质成像对比度的方法在某些场景下不实用，作者希望探索一种基于量子纠缠光子的新途径来替代传统方案。

Method: 使用时空纠缠光子对照射目标，通过符合探测并对空间相关事件进行后选择，分离弹道双光子与散射双光子，同时去除背景光事件。采用数值模拟和两种实验配置（双光子照射场景和单光子照射场景）验证方法。

Result: 两种场景下，相关性后选择均能提高图像对比度，但代价是事件数减少导致散粒噪声增加；通过适当组合多个后选择窗口的事件可部分补偿噪声。

Conclusion: 该方法将弱散射介质成像拓展到量子成像框架，适用于自适应光学、时间门控和空间滤波不实用的场景。
...
TL;DR: 本文表明，局域量子Fisher信息可稳健表征少体开放量子系统中的量子交叉现象，而量子肥胖度不能普遍推广量子操控椭球体积作为交叉指标；在椭球体积对转变不敏感的区域，局域Bloch向量仍能编码交叉特征，揭示了局域与全局指标间的几何差异。


<details>
  <summary>Details</summary>
Motivation: 少体开放量子系统中量子交叉现象的识别通常依赖全局或模型依赖的指标，缺乏基于局域测量的稳健表征方法。

Method: 通过局域量子Fisher信息建立与交叉行为起点的直接联系，分析量子肥胖度与量子操控椭球体积的关系，并研究局域Bloch向量在交叉中的行为。

Result: 量子肥胖度不能普遍推广量子操控椭球体积作为交叉的普适指标；存在椭球体积对交叉不敏感而局域Bloch向量仍能编码相关特征的区域。

Conclusion: 局域与全局指标在表征量子交叉时存在几何差异，局域测量可提供更稳健的交叉识别方法。
...
TL;DR: 提出了一种无需预先知道非幺正传播子闭式表达的量子算法，通过SVD分解将非幺正传播子表示为酉算子之和，并在量子硬件上执行膨胀操作，以模拟非自治线性常微分方程描述的非幺正动力学。


<details>
  <summary>Details</summary>
Motivation: 现有量子算法求解非幺正动力学方程时，需要预先知道非幺正传播子的闭式表达，并依赖经典计算机在每个时间步计算和操作该传播子，这限制了算法的效率和适用范围。本文旨在克服这一局限。

Method: 结合SVD膨胀方案（将非幺正传播子分解为酉算子之和）与在量子硬件上模拟SVD因子的动力学，从而无需经典预计算传播子。

Result: 以描述光诱导电荷转移的population-only time-convolutionless量子主方程为例，展示了算法的适用性及对噪声的敏感性。

Conclusion: 该算法有效避免了经典计算瓶颈，拓展了量子计算在非幺正动力学模拟中的应用范围。
...
TL;DR: 线性储层计算中，当输出使用线性读出时，固定延迟的信息容量受限于预处理输入的容量；线性动力学只能重新分布特征而无法创造新容量。全局容量测度掩盖了这一瓶颈。在协方差基连续变量量子储层中导出对应高斯极限，并实验证明单光子操作可超越该极限，成为量子储层计算的真正资源，同时作为非高斯处理的操作性见证。


<details>
  <summary>Details</summary>
Motivation: 揭示线性储层计算机信息处理容量中隐藏的瓶颈：线性储层动力学在固定延迟下无法创造新表达能力，仅能重新分布特征，而全局容量测度掩盖了这一限制。

Method: 理论推导线性储层在固定延迟下的容量受限性，导出协方差基连续变量量子储层的高斯极限，并通过单光子操作数值实验验证超越该极限。

Result: 线性储层固定延迟容量受限于预处理输入；全局容量测度因累积不同延迟贡献而掩盖瓶颈；单光子操作可超越高斯极限，成为量子储层计算的真正资源。

Conclusion: 线性动力学无法创造新固定延迟表达能力；单光子操作提供的过剩容量可作为连续变量系统非高斯处理的操作性见证。
...
TL;DR: 本文是一篇关于量子纠错（QEC）的综合性教程，系统介绍了从基本概念（码、综合征、稳定子、解码、容错）到主要码族（拓扑码、子系统中、玻色子码、qudit码、动态码、qLDPC码）及当前研究方向的核心知识，旨在为新手和研究人员提供操作性的理解与参考。


<details>
  <summary>Details</summary>
Motivation: 噪声是构建实用量子计算机的核心障碍，量子纠错是保护量子信息免受噪声影响的关键框架。由于量子纠错面临无法复制、直接测量或破坏量子态等独特挑战，且该领域理论和实验进展迅速，亟需一份系统、引导性的教程来帮助新入门者和研究者掌握现代量子纠错的核心概念与最新发展。

Method: 本文采用引导式教程的形式，从底层概念（码、综合征、稳定子形式体系、解码算法、容错阈值）逐层展开，系统地连接到主流码族（拓扑码、子系统中、玻色子码、qudit码、动态码、qLDPC码）和当前研究方向。重点强调操作性理解，即不仅解释核心对象是什么，还说明它们如何用于码设计、错误诊断、解码和容错计算。

Result: 本文提供了一份涵盖经典构造与最新进展的量子纠错综合教程，清晰梳理了QEC的知识体系，将抽象理论与实际工程实现（码设计、解码、容错）相连接，形成了从入门到前沿的完整参考。

Conclusion: 该教程为QEC领域的新手和研究人员提供了系统、连贯的参考，强调对核心概念及其在码设计、解码和容错计算中应用的操作性理解，覆盖了从拓扑码到qLDPC码等多个重要码族及当前研究方向。
...
TL;DR: 本文研究模块化结构对非厄米拓扑系统的影响，发现其能增强非厄米趋肤效应、打破传统体边对应关系，并在谱拓扑相变附近实现临界增强的精密测量灵敏度，且可推广至多参数估计场景。


<details>
  <summary>Details</summary>
Motivation: 非厄米拓扑系统因其独特的物理性质和与开放量子系统的关联而受到广泛关注，本文旨在探索模块化结构（即特定耦合在规则间隔处取不同值）如何丰富和增强这些系统的拓扑属性及传感性能。

Method: 通过理论分析模块化结构对非厄米拓扑系统的影响，研究其对非厄米趋肤效应、体边对应关系以及谱拓扑相变附近传感性能的作用，并拓展到多参数估计场景。

Result: 模块化结构能够丰富非厄米拓扑系统的独特拓扑属性（如增强非厄米趋肤效应、破坏传统体边对应关系），并在谱拓扑相变附近实现临界增强的灵敏度，该增强可推广至多参数估计。

Conclusion: 模块化结构为非厄米拓扑系统提供了新的调控维度，可同时增强其拓扑特性和精密测量灵敏度，具有重要的基础研究价值和实际应用前景。
...
TL;DR: 本文探索了一种针对超弹性材料非线性弹性问题的变分量子公式，采用混合量子-经典框架，通过多项式近似应变能密度，在NISQ设备上实现了基于参数化量子电路的能量泛函评估，并以一维NeoHookean模型为例验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 现有经典数值方法（如有限元法）求解超弹性非线性弹性问题的计算成本高昂，而NISQ量子设备虽有潜力但难以直接处理非线性问题，因此需要探索一种能在近期量子硬件上实现的变分量子方法。

Method: 利用超弹性材料的势能结构，构建混合量子-经典框架：量子部分采用参数化量子电路评估能量泛函，经典部分执行优化；引入应变能密度的多项式近似使其与变分量子算法兼容；采用有限元离散化，以一维NeoHookean模型为例，使用一阶和二阶形函数及非齐次边界条件进行数值实验。

Result: 数值实验展示了多项式近似阶数对方法精度和效率的影响，验证了所提方法在近期量子设备上的可行性。

Conclusion: 该变分量子公式为在NISQ设备上求解超弹性材料非线性弹性问题提供了一种可行的途径，但精度受多项式近似阶数影响，需在计算资源与精度之间取得平衡。
...
TL;DR: 本文研究了低纯度态在非纯化操作下的纠缠生成能力，发现绝对可分态中有一部分可通过概率性协议生成纠缠，并完全刻画了“完全绝对可分”这一新态类，同时提出了仅依赖最大/最小特征值和最小局域维数的新的可分性充分条件。


<details>
  <summary>Details</summary>
Motivation: 实验上制备高纯度态具有挑战性，因此需要探索低纯度态在何种条件下能够产生纠缠，以及哪些态无论如何都无法生成纠缠。

Method: 通过分析绝对可分态在确定性幺正信道下的行为，扩展到概率性非纯化协议，定义并刻画了完全绝对可分态类，并推导了基于谱特征（最大/最小特征值及最小局域维数）的可分性新判据。

Result: 发现某些绝对可分态可以通过概率性协议（不产生纯度）生成纠缠；完全刻画了在任何非零概率下都无法生成纠缠的“完全绝对可分”态类；提出了独立于已知谱可分性判据的新充分条件。

Conclusion: 低纯度态在概率性操作下仍有可能生成纠缠，但完全绝对可分态构成了一个无法生成纠缠的极限类；新的谱可分性判据为纠缠理论提供了更简洁的判定工具。
...
TL;DR: 本文提出一种混合高斯-指数外推方案，用于周期性量子电路的零噪声外推误差抑制，通过证明噪声放大因子弱收敛于对数正态分布，在中等至大深度电路上相比传统指数模型显著降低偏差。


<details>
  <summary>Details</summary>
Motivation: 零噪声外推的精度高度依赖于噪声模型与实际误差缩放之间的保真度，现有指数模型对周期性电路的误差刻画不够精确，需要更适配的噪声模型来提升误差抑制效果。

Method: 引入混合高斯-指数外推方案；在Pauli对角误差下，通过构建和分析Pauli算子传递的近似马尔可夫过程，证明噪声放大因子弱收敛于对数正态分布，从而在标准指数模型上增加高斯方差修正项。无需先验噪声表征，直接适用于任意周期性量子电路。

Result: 在Trotterized Ising动力学、随机电路和Grover搜索算法的Qiskit噪声模拟器上验证，对于中等至大深度电路，混合模型相比此前外推变体实现了可测量的偏差降低。

Conclusion: 混合高斯-指数外推方案能有效提升近期量子硬件上周期性电路的误差抑制效果，具有实用价值。
...
TL;DR: 本文对基于置换矩阵表示（PMR）的哈密顿量模拟方法与其他主流量子算法进行了资源成本对比研究。在时间无关情况下，以里德伯相互作用哈密顿量为例，比较了PMR与量子信号处理（QSP）和量子比特化方法；在时间相关情况下，将PMR的时间相关扩展与量子高振荡协议（qHOP）在任意维度Floquet驱动横场伊辛模型上进行了比较。结果表明PMR在资源需求上具有互补优势，并在某些系统参数下展现出有利的缩放特性，可能在资源受限的量子硬件上提供实际优势。


<details>
  <summary>Details</summary>
Motivation: 现有哈密顿量模拟算法（如QSP、qubitization、qHOP）在不同场景下有各自的优缺点，但缺乏对PMR方法系统性的资源成本对比研究。本文旨在填补这一空白，评估PMR在资源受限量子硬件上的实际应用潜力。

Method: 采用比较研究的方法，对PMR与其他量子算法进行资源成本分析。时间无关情况下，以里德伯相互作用哈密顿量为基准，对比PMR与QSP、qubitization；时间相关情况下，以Floquet驱动横场伊辛模型为基准，对比PMR的时间相关扩展与qHOP协议。

Result: PMR在资源需求上表现出互补优势，并在某些系统参数下具有更优的缩放行为。

Conclusion: PMR方法可能在资源受限的量子硬件上提供实际的计算优势，作为一种有竞争力的哈密顿量模拟工具。
...
TL;DR: 本文提出了一种基于广义迹动力学（GTD）的CSL型坍缩噪声关联子的条件性构造方案，通过引入辅助规范费米子Fock空间扇区和标量化假设，推导出哈勃尺度的窄带谱，该谱线位于现有CSL实验约束范围之外，因此与现有高频数据不冲突。


<details>
  <summary>Details</summary>
Motivation: 旨在从GTD框架出发，在不依赖现象学拟合的条件下，通过一系列显式结构公设推导候选CSL型坍缩噪声算符的谱形式与振幅，并检验其与现有实验数据的兼容性。

Method: 在最小GTD Grassmann代数中，由于纯费米子系数的幂零性导致迹约化受阻，作者引入辅助正则费米子Fock空间扇区替代原始幂零系数，通过标量双线性J作为热库算符、正模正则量子化以及有效符号选择σ=±1，利用Wick缩并计算两点函数，最终得到频率为|ω|=2ω₀的Wightman谱线。

Result: 得到关联子振幅为A_J = (ħ/2m_R ω₀ L_aik²)²·N·D，谱线位于|ω|=2ω₀处。通过宇宙学识别ω₀∼H₀，该谱线位于两倍哈勃尺度，且处于当前CSL实验的频段约束之外，具有可验证的窄带特征。

Conclusion: 该条件性构造为GTD框架与CSL坍缩模型之间的桥梁提供了具体计算路径，所得谱线避开现有高频实验约束，为未来实验检验提供了明确预言。
...
TL;DR: 本文提出混合态t-反相干性的公理化框架，区分总反相干性（在SU(2)协变通道下非减）与量子反相干性（资源单调量），并构造基于纯化、希尔伯特-施密特距离和多极矩的度量体系，通过凸包扩展建立量子对应，分析混合态示例、最大量子反相干态及纯度与反相干阶数的权衡。


<details>
  <summary>Details</summary>
Motivation: 混合态反相干性的各向同性可能源于量子关联或经典统计混合，现有研究多聚焦纯态，缺乏混合态的统一公理化框架和资源理论。

Method: 基于对称量子比特嵌入建立混合态t-反相干公理框架；定义总反相干性（SU(2)协变通道下非减）与量子反相干性（资源单调量，纯态与总度量一致）；构造总度量（约化态纯度、Hilbert-Schmidt距离、累积多极矩）及量子对应（凸包扩展纯态泛函，关联对称扇区二分纠缠）；分析最大量子反相干态、粒子丢失鲁棒性和纯度-反相干阶权衡。

Result: 建立了混合态反相干性的公理化资源理论；区分并构造了总与量子两类度量；给出了显式混合态示例；确定了反相干子空间上具有最大量子反相干性的态；表征了粒子丢失对不同类型态的影响及纯度与反相干阶数的权衡关系。

Conclusion: 混合态反相干性可分解为量子贡献与经典贡献；所提框架为方向无关计量学和量子参考系对准提供了更一般的资源理论工具。
...
TL;DR: 提出一种混合ROI细化框架，先通过经典或量子方法进行粗全局重建，再对选定ROI进行量子优化，在减少量子变量数量的同时实现高保真重建。实验表明SART+QTR/QCSTR管线在ROI重建中表现最优，量子CT重建的实际优势在于局部细化而非全局重建。


<details>
  <summary>Details</summary>
Motivation: 量子优化用于CT重建受限于图像表示所需的二进制变量数量，直接进行全图像量子重建对于大尺寸或结构复杂的物体十分困难，因此需要一种能减少量子变量规模同时保持重建质量的策略。

Method: 提出混合ROI细化框架：第一阶段使用QTR/QCSTR、FBP或SART进行粗全局重建；第二阶段通过残差投影-图像公式，仅对选定ROI应用量子优化（QTR/QCSTR），从而有效减小QUBO问题规模。

Result: 在三个离散体模上的实验表明：QTR/QCSTR+QTR/QCSTR和SART+QTR/QCSTR在减少角度设置下均能实现中等尺寸样本的准确ROI重建；对于最大最复杂样本，稳定的经典粗重建结合第二阶段ROI-only量子优化效果最佳；SART+QTR/QCSTR管线实现了最低的平均ROI RMSE和MAE。

Conclusion: 量子辅助CT重建的实用优势在于将量子优化保留用于局部细化，同时使用经典重建来稳定全局背景，而非追求全图像的量子重建。
...
TL;DR: 氢终端金刚石表面的吸附受体与氮替代施主形成光活性施主-受主对（DAP），通过CVD中间层可精确调控施主-受主间距（纳米精度），产生明亮窄谱线的单光子发射，揭示了表面转移掺杂的光学对应，建立了可纳米调谐的DAP量子发射器平台。


<details>
  <summary>Details</summary>
Motivation: 探索氢终端金刚石表面吸附受体与体相氮施主之间是否存在光学活性施主-受主对（DAP）复合，并建立可精确控制间距的量子发射器平台。

Method: 在富氮HPHT衬底与氢终端表面之间插入名义上非掺杂的CVD中间层，以纳米精度调控施主-受主间距；测量不同中间层厚度下的辐射DAP复合的光谱强度、能量和衰减动力学，并分析单光子统计和偏振各向异性。

Result: DAP复合呈现明亮、窄谱线的发射，其强度、能量和衰减动力学随中间层厚度系统性变化；单个DAP线显示单光子统计， ensembles表现出强烈的偏振各向异性，反映了平面的施主-受主几何结构。

Conclusion: 发现了氢诱导表面转移掺杂在金刚石中的光学对应，建立了表面定义的、纳米可调谐的DAP量子发射器平台。
...
TL;DR: 本文在双协方差模型（DCM）基础上发展了交互扩展版本，通过多尺度Itô演算和滑动窗口平均从局部随机微分方程推导出粗粒化双协方差算子的封闭演化方程，并在流体动力学极限下证明宏观动力学收敛到精确的GKSL形式，为开系统和闭系统量子动力学提供了统一的随机基础。


<details>
  <summary>Details</summary>
Motivation: 在随机亚量子框架下，将双协方差模型从非交互情形扩展到包含交互作用的更一般形式，建立微观随机涨落与宏观量子动力学之间的严格数学联系，统一描述开系统和闭系统的量子动力学。

Method: 采用多尺度Itô演算和滑动窗口平均方法，在子系统希尔伯特空间上建立局部随机微分方程，引入快速微观涨落尺度和慢速宏观观测尺度的分离，通过流体动力学极限分析粗粒化动力学方程的收敛行为。

Result: 在流体动力学极限下，快速涨落修正消失，有效动力学收敛到确定性的宏观输运方程，该方程具有精确的GKSL形式：确定性亚量子流产生相干哈密顿演化，二次噪声关联产生耗散通道。还证明了不可分离的交互哈密顿量可从严格的局部、状态依赖随机反馈场产生。在无涨落极限下，模型自然退化为标准冯·诺依曼方程。

Conclusion: 该工作为量子动力学提供了统一的随机基础框架，从亚量子随机过程中涌现出标准量子力学中的哈密顿演化和GKSL耗散结构，弥合了微观随机性与宏观量子确定性之间的鸿沟。
...
TL;DR: 本文研究了BB84、SARG04和BBM92三种水下量子密钥分发协议在清澈、沿海和浑浊三种非湍流海水中的性能，通过量子误码率和量子关联性评估了系统光学参数和水下信道物理效应的影响，并利用Kraus算子对BBM92协议进行建模，通过蒙特卡洛仿真验证了结果，确定了各协议实现最优性能的条件。


<details>
  <summary>Details</summary>
Motivation: 探索水下环境中量子密钥分发协议的可行性，分析不同水质和系统参数对通信安全性和效率的影响，为水下量子通信的工程实践提供理论依据。

Method: 采用BB84、SARG04和BBM92三种QKD协议，以量子误码率和量子关联性为性能指标，在三种非湍流海水（清澈、沿海、浑浊）中评估传播距离的影响；利用Kraus算子对BBM92协议建模以描述衰减和消偏极效应对最大纠缠光子的联合影响，并推导了QBER的解析表达式；通过蒙特卡洛仿真进行验证。

Result: 蒙特卡洛仿真结果验证了所有协议在不同水质、大气条件和系统参数下的QBER解析表达式，确定了各QKD协议实现最优性能的条件。

Conclusion: 本研究为水下安全高效量子通信的协议选择与参数优化提供了理论指导，明确了不同海水环境下各QKD协议的最佳工作条件。
...
TL;DR: 本文推导了由Pauli字符串生成的参数化酉门的Rademacher复杂度界限：全参数域为𝒪(L^{3/2}/√M)，受限参数域为𝒪(L/√M)，并通过数值实验验证了标度规律。


<details>
  <summary>Details</summary>
Motivation: 尽管量子机器学习模型的泛化界限已在多个场景中被研究，但由Pauli字符串生成的参数化酉门的显式Rademacher复杂度界限仍不够清晰，本文旨在填补这一空白。

Method: 理论推导了参数化酉门的Rademacher复杂度标度界限（全参数域和受限参数域），并与经典线性模型类进行比较，辅以数值实验进行定性验证。

Result: 全参数域Rademacher复杂度上界为𝒪(L^{3/2}/√M)，受限参数域上界为𝒪(L/√M)。与经典线性模型对比发现，当经典模型的输入范数和参数范数均随参数数量标度时，量子模型可能存在统计复杂度优势。

Conclusion: 本文为Pauli生成参数化酉门提供了清晰的Rademacher复杂度界限，揭示了潜在的量子和经典模型间的统计复杂度差异。
...
TL;DR: 本文基于宏观量子电动力学和纳米光子学中的少模量子化方法，建立了腔体相干散射构型下光阱反冲加热率的预测理论，发现电磁环境可通过Purcell效应显著抑制反冲加热率，突破了传统认为反冲加热率近似等于自由空间值的假设，为通过光子结构设计抑制悬浮光力学中的运动退相干提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 在光学捕获中，光子散射引起的反冲加热是运动退相干的基本来源，严重限制了非经典运动态的制备。腔体相干散射构型下，由于尖锐光学共振的存在，传统微扰方法失效，缺乏反冲加热率的预测理论，当前工作只能近似认为反冲加热率等于自由空间值。本文旨在证明这一近似不正确，并建立通用理论框架。

Method: 基于宏观量子电动力学和纳米光子学中的少模量子化方法，建立了适用于任意电磁结构下光阱粒子的通用理论框架。

Result: 电磁环境可通过Purcell效应显著调控反冲加热率；在先进微腔中，质心运动和摆动运动的反冲加热率均可被显著抑制。

Conclusion: 传统认为反冲加热率近似等于自由空间值的假设不成立；所建立的理论方法适用于任意电磁结构，为通过光子结构设计工程化调控悬浮光力学中的运动退相干提供了可行路径。
...
TL;DR: 本文提出了一种称为QAVG（可变网格平均量子相位估计）的游标式方法，通过结合低分辨率QPE、多起点偏移和物理驱动的连续参数化，在噪声环境下精确重建多电子系统的激发谱。实验在Quantinuum H2-2硬件上进行，包括物理和逻辑QPE电路，证明了即使在噪声直方图下，QAVG也能以远低于标称QPE分辨率的误差重建光谱，有效抑制了光谱泄漏引起的局部极小值，为早期容错量子计算机时代的量子模拟提供了稳健路径。


<details>
  <summary>Details</summary>
Motivation: 量子相位估计（QPE）是提取多电子系统激发谱的关键技术，但在当前硬件上受限于低网格分辨率和环境噪声，实际应用面临挑战。现有方法难以在噪声环境下实现高精度光谱重建。

Method: 提出QAVG方法，采用游标卡尺式策略：将低分辨率QPE与多个原点偏移相结合，并引入物理驱动的连续参数化来精确重建光谱。将其集成到基于第一性原理的CO分子吸附在χ-Fe₅C₂表面的端到端工作流中，在Quantinuum H2-2上分别使用物理QPE电路和Steane编码的逻辑QPE电路（带离线比特翻转纠错）进行实验验证。

Result: QAVG能够精确重建光谱，偏差远小于标称QPE分辨率，即使在噪声直方图下仍能保持高精度。偏移网格上的平均代价景观有效抑制了光谱泄漏引起的局部极小值，稳定了试验参数的优化过程。

Conclusion: QAVG为早期容错量子计算机时代的相关谱量子模拟提供了一条稳健的技术路线，有望推动量子计算在实际材料科学和化学问题中的应用。
...
TL;DR: 提出了一种用于量子计算机处理关联电子系统的量子-经典混合动力学平均场理论(QAVG-DMFT)方案，其中量子部分使用改进的量子相位估计电路提取有限温度下的单粒子格林函数，经典部分采用QAVG方法基于量子采样数据重建格林函数，并通过SrVO₃数值模拟验证了方案的有效性。


<details>
  <summary>Details</summary>
Motivation: 在量子计算机上处理关联电子系统时，需要有效的方案来求解动力学平均场理论(DMFT)，特别是克服传统量子相位估计在提取有限温度单粒子格林函数时无法区分不同激发通道的困难。

Method: 量子部分：使用改进的量子相位估计(QPE)电路，能够在有限温度下提取单粒子格林函数的谱振幅和激发能量，且无需在每次测量时区分具体激发通道。经典部分：采用QPE变网格平均(QAVG)方法，通过优化试探参数和建模不同QPE电路设置下的概率分布来重建格林函数。

Result: 将QAVG-DMFT方案应用于SrVO₃，通过数值模拟验证了该方案的有效性。

Conclusion: 提出的量子-经典混合QAVG-DMFT方案可有效在量子计算机上处理关联电子系统，为实际材料计算提供了可行的混合计算框架。
...
TL;DR: 本文对基于地球同步轨道（GEO）卫星的量子密钥分发（QKD）下行链路进行了全面的端到端可行性研究，采用诱骗态BB84协议、可变长度有限密钥安全性和紧致统计界，结合物理信道模型和欧洲历史云数据，评估了多种波长、接收架构和环境下的年密钥产量，并识别了关键性能瓶颈与设计准则。


<details>
  <summary>Details</summary>
Motivation: 实现基于GEO卫星的连续、大尺度安全通信，但面临极端信道损耗和背景噪声（尤其是白天运行）的挑战，需系统性评估其可行性。

Method: 采用诱骗态BB84协议，结合可变长度有限密钥安全性和紧致统计界；涵盖多种接收器架构；使用物理信道模型模拟损耗与噪声；评估可见光Fraunhofer吸收谷和电信波段；利用欧洲历史云数据预测年密钥产量。

Result: 确定了不同环境（农村、城市、沿海）和波长下的性能权衡与瓶颈，建立了可行的运行阈值，给出了未来GEO-QKD任务的设计指导。

Conclusion: GEO下行QKD在适当设计下具备可行性，研究结果为实际系统设计提供了量化依据和操作指南。
...
TL;DR: 本论文研究有限维量子系统中非经典资源的识别、表征和噪声保护，利用离散Wigner函数框架引入负量子态，分析其在非马尔可夫噪声下的演化，提出基于弱测量和量子测量反转的保护策略，并在IBM量子硬件上实现制备和验证，证明负量子态在某些噪声场景下优于Bell态。


<details>
  <summary>Details</summary>
Motivation: 在真实噪声环境中，如何识别、表征并保护有限维量子系统中的非经典资源，使其能够实际应用。

Method: 基于离散Wigner函数（DWFs）框架定义负量子态；分析非马尔可夫随机电报和振幅阻尼动力学下的演化；使用弱测量和量子测量反转策略保护量子关联；在IBM量子硬件上设计电路制备并用量子态层析验证。

Result: 弱测量和量子测量反转可增强量子关联、降低保真度偏差、提升隐形传态性能；源于相空间点算子的负量子态在非马尔可夫噪声下纠缠鲁棒性优于Bell态；在IBM量子硬件上成功制备并验证高保真度；两比特负量子态在CHSH不等式的最大违背和Fisher信息方面优于Bell态。

Conclusion: 负量子态有潜力在非马尔可夫错误占主导的场景中替代传统Bell态作为量子资源。
...
TL;DR: TW2S是一种基于图结构的两阶段门切割选择方法，通过在电路交互图上分析树宽上界和边介数中心性来识别路由瓶颈，在稀疏连接的超导量子设备上有效降低长程两比特门带来的SWAP开销和错误率。


<details>
  <summary>Details</summary>
Motivation: 在稀疏连接的量子设备上，长程两比特门的转译会插入大量SWAP门，增加硬件成本和执行错误。虽然门切割（gate cutting）可以通过准概率分解移除选定的两比特门以减少路由开销，但其采样成本高昂，因此切割位置的选取至关重要。

Method: 提出TW2S双阶段门切割选择方法：阶段1分析最小填充消除轨迹，根据边对树宽上界的贡献评分；阶段2结合边介数中心性和度惩罚对候选边排序。该方法仅基于电路交互图操作，无需选择时进行后端特定的转译。

Result: 在网格图、Watts-Strogatz、barbell和随机块模型基准测试中，TW2S在交互图存在可辨识稀疏切割时一致优于随机切割选择。在J1-J2横场Ising模型噪声模拟中，TW2S在n=8时实现ΔECR=47（随机选择约9），在强信号区域误差低于未切割基线。

Conclusion: 图结构切割选择可作为实用的编译器端工具，将电路切割转化为有针对性的路由减少策略。推导的均方误差收支平衡条件表明，在共享总采样预算下，QPD仅在ECR减少足够大且信号强度充足时有益；在扩展的子电路预算下信号强度要求大幅放宽。
...
TL;DR: 本文研究了单个量子发射器耦合到玻色型Su-Schrieffer-Heeger（SSH）浴的局域相位估计问题，发现二聚化打开的中心能隙支持发射器-浴束缚态，从而抑制发射器相干性的完全弛豫，并分析了相位量子Fisher信息与能隙、失谐和耦合强度的关系。


<details>
  <summary>Details</summary>
Motivation: 探索在拓扑非平凡浴（SSH链）中，单个量子发射器的局域相位估计性能，特别是二聚化引起的能隙如何影响相位信息的保持和计量学响应。

Method: 建立微观晶格模型，通过Dyson方程分析局域浴格林函数、束缚态条件及发射器残余权重；利用量子Fisher信息量化相位估计的计量学性能。

Result: 共振时强耦合增强瞬态杂化但降低保留信号；失谐时相位信息保护随发射器调谐至带边而减弱；均匀链对照显示能隙闭合时保留信号消失；响应仅依赖于能隙和束缚态，不敏感于SSH绕数符号。

Conclusion: 该协议探测的是谱能隙物理和束缚态支撑，而非拓扑绕数扇区。
...
TL;DR: 本文推导了Haar随机纯态的平均双体量子互信息$\langle I(A:B)\rangle$的精确非微扰闭式：$\langle I(A:B)\rangle = (d_A^2-1)(d_B^2-1)\,\mathcal{G}(d_A,d_B,d_E)$，其中$\mathcal{G}$由Bose-Einstein核的收敛积分显式给出。该表达式精确而非渐近，其渐近展开是发散的Bernoulli因式化级数（涉及$\zeta(1-2k)$），而积分恰为其精确Borel和。通过核的尺度反演对称性，可直接得到$\langle I\rangle < (d_A^2-1)(d_B^2-1)/(2N)$。推导将Page熵精确分解为对角（Dirichlet）贡献与Schur优超本征值修正，二者组装为互信息时清晰分离了经典与量子关联。


<details>
  <summary>Details</summary>
Motivation: Haar随机纯态的平均双体量子互信息此前仅能通过Page公式以digamma函数形式表达，缺乏简洁的非微扰闭式。本文旨在给出该量的精确闭式解，并揭示其与李代数维数、Bose-Einstein分布及Borel可和性的深层联系。

Method: 将Page熵精确分解为对角（Dirichlet）贡献与Schur优超本征值修正；通过Gamma函数积分表示及Bose-Einstein核的尺度反演对称性，将平均互信息表达为收敛积分；利用Borel可和性处理渐近展开的发散级数。

Result: 获得了平均互信息的精确闭式$\langle I(A:B)\rangle = (d_A^2-1)(d_B^2-1)\,\mathcal{G}(d_A,d_B,d_E)$，前因子为$\mathfrak{su}(d_A)\times\mathfrak{su}(d_B)$维数之积；$\mathcal{G}$的渐近展开为发散Bernoulli级数（系数含$\zeta(1-2k)$），其Borel和即为原积分；通过核对称性证明了上界$\langle I\rangle < (d_A^2-1)(d_B^2-1)/(2N)$。

Conclusion: 平均互信息的精确结构源于Page熵的对角与优超修正分解，该分解自然区分了经典与量子关联；积分表示不仅提供了非微扰闭式，还揭示了发散级数的Borel可和性，为理解随机纯态量子关联的代数-统计结构提供了新视角。
...
TL;DR: 本文从超球面上的精确投影中心极限定理出发，重新推导了子系占据概率的Beta分布和Lubkin纯度公式，并给出了Haar随机纯态下两体量子互信息$\langle I(A:B)\rangle$的完整渐近展开及非微扰闭合形式。


<details>
  <summary>Details</summary>
Motivation: 量化有限尺寸下"platykurtic"尾抑制相对于高斯近似的偏差，并解决Haar随机纯态下两体量子互信息的精确渐近展开问题。

Method: 利用超球面投影中心极限定理和初等超球面矩重新推导Beta分布和Lubkin公式；通过Page公式的精确代数重组，将其分解为量子相干贡献和经典概率贡献，并借助Schur优超定理和Cartan结构给出维度计数$(d-1)$和$(d^2-1)$的几何意义。

Result: 互信息的完整渐近展开在$1/N$中具有Bernoulli因子化形式，每阶$k\ge1$带有对称因子$(d_A^{2k}-1)(d_B^{2k}-1)$，所有奇阶修正为零。主导有限尺寸修正分离为$\mathfrak{su}(d_A)\otimes\mathfrak{su}(d_B)$双体重子相干贡献$(d_A^2-1)(d_B^2-1)/(2N)$和减去的经典概率（Cartan$\otimes$Cartan）贡献$(d_A-1)(d_B-1)/(2N)$。

Conclusion: 给出了互信息的非微扰闭合形式$\langle I(A:B)\rangle = (d_A^2-1)(d_B^2-1)\,\mathcal{G}(d_A,d_B,d_E)$，其中$\mathcal{G}$为显式Bose-Einstein积分，其$1/N$渐近展开重现Bernoulli级数。
...
TL;DR: 本文提出了一种量子算法，通过高效的量子比特编码方案同时计算Kohn-Sham DFT中的所有占据轨道，并利用Harris泛函避免电子密度读取，有望实现指数级加速；同时提出了另一种基于多份波函数副本的自洽DFT计算方法，并通过数值示例验证了算法的有效性。


<details>
  <summary>Details</summary>
Motivation: 量子计算在电子结构计算中展现出潜力，但其对密度泛函理论（DFT）计算能否实现加速尚不明确，作者希望探索量子算法在DFT中的加速可能性。

Method: 1) 提出了一种量子比特高效的波函数编码方案，用于Kohn-Sham DFT；2) 设计了一种可同时计算所有占据轨道的量子算法；3) 利用Harris泛函规避传统DFT中昂贵的电子密度读取步骤，实现潜在的指数级加速；4) 提出了基于多份波函数副本的自洽DFT计算方法，同样避免了密度读取。

Result: 通过数值示例验证了所提出算法的适用性，并与现有方法进行了效率对比。

Conclusion: 所提出的量子算法能够有效加速DFT计算，特别是结合Harris泛函时具有指数级加速潜力，两种方法均规避了电子密度读取这一瓶颈。
...
TL;DR: 本文证明任何完备的正交乘积基如果在初始状态下是局域可区分的，那么它在所有保持正交性的局域投影测量下仍保持可区分性，从而排除了通过保持正交性的局域投影测量和经典通信激活隐藏非局域性的可能性。进一步引入并形式化了"强局域集"的概念，即在所有二分划分下均不可被激活的局域可区分集合。


<details>
  <summary>Details</summary>
Motivation: 研究正交乘积态在局域操作和经典通信下的可区分性，特别是探索"无纠缠的量子非局域性"现象中隐藏非局域性的"真正激活"机制，并厘清局域可区分性在正交保持变换下是否可能不可逆地丧失。

Method: 通过理论分析证明完备正交乘积基在保持正交性的局域投影测量下保持可区分性，并引入"强局域集"形式化定义以刻画不可被激活的局域可区分集合。

Result: 1) 任何初始局域可区分的完备正交乘积基在正交保持的局域投影测量下仍可区分，排除了该路径下的激活；2) 建立了"强局域集"的概念，作为刻画LOCC可区分性与其不可逆丧失之间边界的工具。

Conclusion: 为量子态区分中局域到非局域转变提供了严格的结构性理解，有助于界定多体系统中LOCC可区分性的边界及其不可逆丧失的条件。
...
TL;DR: 本文提出了量子层析成像中的分辨率框架，指出有效分辨率由与测量Gram矩阵相关的采样算子决定，该算子定义了实验可访问的自由度和量子态的重建带宽，类似于成像中的传递函数，为区分真实量子特征与伪影提供了操作准则。


<details>
  <summary>Details</summary>
Motivation: 量子层析成像能够重建精细的相空间结构，但这些结构不一定被测量本身所分辨。需要建立一个框架来区分哪些特征是真实可分辨的，哪些是由不完全采样或重建假设引入的伪影。

Method: 通过分析测量Gram矩阵定义的采样算子，将其类比为成像中的传递函数，并引入Gram本征基下的重建作为测量自适应压缩方法。同时指出该结构在有限框架理论中自然对应于框架算子。

Result: 建立了基于分辨率的量子层析成像框架，明确了有效分辨率由采样算子决定，Gram本征基重建是高效的测量自适应压缩方案。

Conclusion: 该分辨率框架为当代探测高度结构化非经典态的量子实验提供了理论依据和操作准则。
...
TL;DR: 本文研究了量子密码学中安全证明所需的条件熵链式法则，发现设备无关（DI）场景下自然紧化链式法则不可行，但提出了一个中间改进版本，并在Rényi熵累积定理中实现了略紧的界。


<details>
  <summary>Details</summary>
Motivation: 量子密码学的安全证明依赖条件熵，但在多轮协议中需应对最一般攻击（非i.i.d.）。链式法则通过将结构化非i.i.d.过程的条件熵分解为各轮熵贡献之和来解决此问题。现有研究在可信设备场景下已实现有限轮数的紧i.i.d.约化，但设备无关（DI）场景下是否能恢复类似结果尚未被研究。

Method: 1. 证明Dupuis等人链式法则的自然紧化在DI场景下不可能成立，揭示当前DI安全证明方法的局限性。2. 提出DI场景下的新链式法则，实现中间改进。3. 基于Arqand等人的框架，将该链式法则应用于Rényi熵累积定理，得到略紧的版本。4. 提供统一框架，梳理现有链式法则并比较其应用。

Result: 1. 证明DI场景下链式法则的自然紧化不可行。2. 提出了一个更优的链式法则（虽非紧的）。3. 在特定上下文中得到了略紧的Rényi熵累积定理版本。4. 建立了统一现有链式法则的框架。

Conclusion: 揭示了DI安全证明中链式法则紧化的根本限制，同时通过新链式法则实现了部分改进，为DI量子密码学的安全证明提供了更优的工具。
...
TL;DR: 该论文针对稳定子态保真度认证中的规范依赖问题，提出了自适应扩展方法以报告完整的认证保真度区间。对于单一规范，推导了互补的最优最坏情况上界；将规范选择建模为自适应设计问题，通过见证消除策略解决线性规划并选择新规范；证明了单调收紧、完全覆盖时的精确恢复及其最坏情况必要性；识别了自适应方法可超越指数基准的结构化综合征分布，并通过数值实验验证了更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有稳定子态保真度认证方法在固定规范下给出最优最坏情况保真度下界，但由于规范依赖导致保真度存在较大歧义，缺乏完整的认证区间。

Method: 推导了单一规范下的互补最优最坏情况上界；将规范选择转化为自适应设计问题，每轮求解精确端点线性规划并通过见证消除策略选择新规范；证明单调收紧性质及完全覆盖时的精确恢复。

Result: 证明自适应方法单调收紧保真度区间，完全覆盖所有非平凡稳定子时可精确恢复；识别出结构化综合征分布下自适应可超越指数基准，数值验证了更快的收敛速度。

Conclusion: 提出的自适应扩展方法有效解决了规范依赖导致的保真度歧义问题，能够报告完整的认证保真度区间，并在结构化条件下展现出超越传统基准的性能。
...
TL;DR: 本文研究了两个囚禁$^{40}$Ca$^+$离子在纳秒级激发脉冲下产生的拉曼光子不可区分性，揭示了自发散射回到初始态对Hong-Ou-Mandel干涉的影响，并发现平均后向衰变次数作为可测量的单发射体参数，与两个相同发射体光子干涉可见度相关。


<details>
  <summary>Details</summary>
Motivation: 探究在纳秒级激发脉冲下，囚禁离子产生的拉曼光子不可区分性，特别是理解自发散射回到初始态这一过程对光子干涉质量的影响机制。

Method: 利用两个囚禁的$^{40}$Ca$^+$离子产生拉曼光子，采用纳秒级短脉冲激发，通过Hong-Ou-Mandel干涉实验测量光子的不可区分性，分析后向衰变次数与干涉可见度的关联。

Result: 确定了平均后向衰变次数这一可测量的单发射体参数，该参数与两个相同发射体产生的光子之间可实现干涉可见度存在直接关联。

Conclusion: 自发散射回到初始态的过程显著影响拉曼光子干涉质量，平均后向衰变次数可作为预测光子不可区分性的关键单发射体指标。
...
TL;DR: 本文提出以门参数化的Lee-Yang零点作为有限量子电路中动力学相位的探针，通过砖墙模型展示Loschmidt振幅可表示为门参数的有理函数，零点在复参数平面上的凝聚曲线由等模Floquet特征值（Beraha-Kahane-Weiss定理）和态依赖贡献共同决定，参数变化时零点集发生突变，提供有限量子比特的动力学相变诊断。


<details>
  <summary>Details</summary>
Motivation: 现有动力学相变诊断方法在有限尺寸量子电路中面临挑战，需要一种不依赖热力学极限或可积性的通用探针。本文旨在利用门参数化的Lee-Yang零点作为有限量子电路中动力学相变的有限尺寸诊断工具。

Method: 采用砖墙模型，时间演化由Floquet算子的重复作用生成。将Loschmidt振幅表示为门参数的有理函数，固定系统尺寸和大深度，在复化门参数平面上研究零点的凝聚行为。利用Beraha-Kahane-Weiss定理分析等模Floquet特征值对应的通用分量，同时考虑初态重叠的态依赖贡献。

Result: 零点在复门参数平面上凝聚成极限曲线，包含等模Floquet特征值决定的通用分量和初态决定的态依赖分量。参数变化时零点集发生突变，可作为有限尺寸量子电路中动力学相变的诊断信号。该机制不依赖可积性，仅需谱竞争和局域幺正性。

Conclusion: 门参数化的Lee-Yang零点为有限量子电路中的动力学相变提供了一种有效的有限尺寸诊断工具，其普适性源于谱竞争和局域幺正性而非可积性。
...
TL;DR: 本文研究了一种无需精確模式匹配的雙向隱蔽光通信方案，通過窄帶激光源實驗驗證了概念，並提出了一種基於相關器的接收器來實現寬帶增益。


<details>
  <summary>Details</summary>
Motivation: 傳統寬帶光量子隱蔽通信雖理論上能實現高隱蔽吞吐量，但模式匹配和相位同步要求過高，難以實際應用。

Method: 採用窄帶激光源進行實驗演示，並提出基於相關器的接收器設計，以規避寬帶量子光源所需的精確模式匹配問題。

Result: 成功實現了雙向隱蔽通信系統的概念驗證，相關器接收器達到了寬帶增益效果。

Conclusion: 窄帶激光源結合相關器接收器可作為一種實用化的隱蔽通信方案，降低了對模式匹配的嚴格要求，同時保持了良好的隱蔽通信性能。
...
TL;DR: 系统审查81篇量子误差缓解(QEM)论文发现，仅25%使用推断性统计方法，通过零噪声外推(ZNE)案例研究揭示了参数敏感性和时间漂移导致的人为假象，并提出最低报告标准。


<details>
  <summary>Details</summary>
Motivation: QEM被视为从NISQ设备过渡到FTQC的关键技术，但其有效性评估的实证研究方法缺乏严格审查，现有结论的有效性存疑。

Method: 采用八准则框架对81篇近期QEM论文进行系统综述，涵盖统计严谨性、可重复性和报告质量；以ZNE为典型案例，通过132种参数配置扫描和72小时纵向硬件实验识别人为假象来源。

Result: 仅15篇(25%)使用推断性方法，25篇(42%)仅描述性报告不确定性；ZNE案例中参数选择可导致结论从显著改进变为显著退化，时间漂移使效应量变化超过三倍并减少有效独立观测数。

Conclusion: 当前QEM评估实践使缓解性能显得比证据支持的更稳健，需建立包含参数文档、鲁棒性检查、纵向漂移评估和推断性统计检验的最低报告标准。
...
TL;DR: 本文研究了非球形中性粒子在自由空间中旋转时与电磁量子真空的相互作用。当旋转轴与粒子对称轴正交时，散射场会产生频率边带，导致动力学卡西米尔光子对的参量发射。在最大尖端速度的结构约束下，近球形几何结构可最大化发射率，并在极化激元共振附近进一步增强。然而，即便这些优化上限，对于现实材料参数而言仍然极小，从而为单个纳米颗粒的自由空间旋转动力学卡西米尔发射设置了严格的数量极限。


<details>
  <summary>Details</summary>
Motivation: 探索非球形中性粒子在自由空间中旋转时与电磁量子真空的相互作用，特别是研究旋转动力学卡西米尔效应的可能性及其理论极限。

Method: 理论分析非球形中性粒子在自由空间中旋转时与电磁量子真空的相互作用，考虑旋转轴与粒子对称轴正交的情形，分析散射场的频率边带和动力学卡西米尔光子对的参量发射，并在最大尖端速度约束下优化几何结构和材料参数。

Result: 近球形几何结构可最大化发射率，极化激元共振可进一步增强，但即便这些优化上限，对于现实材料参数而言仍然极小。

Conclusion: 为单个纳米颗粒的自由空间旋转动力学卡西米尔发射设置了严格的数量极限，表明该效应在现实中极其微弱。
...
TL;DR: 本文评估了FALQON算法中参数迁移策略对Max-Cut问题的效果，发现迁移成功与否主要由接收图决定而非捐赠图，密集接收图表现优异，且小规模捐赠图（8节点）即可匹配更大规模实例的性能，大幅降低测量开销。


<details>
  <summary>Details</summary>
Motivation: FALQON算法需要大量测量来通过反馈循环更新参数，计算开销大。本研究旨在探索能否通过迁移学习的方式，将小规模图优化好的参数直接迁移到更大规模的图上，从而减少测量开销。

Method: 从8、10、12节点的小规模捐赠图计算FALQON参数序列，直接应用到14节点接收图。使用3-正则图和Erdős-Rényi随机图两类图族，测试同族和跨族的参数迁移效果。

Result: 1) 迁移成功取决于接收图结构而非捐赠图；2) 密集接收图无论来源都能获得高近似比；3) 稀疏跨族迁移仍具挑战性；4) 8节点捐赠图与更大规模实例性能相当。

Conclusion: 廉价的小规模图可以为更大目标图提供鲁棒参数，显著降低FALQON反馈循环的测量开销。
...
TL;DR: 本文证明了Pauli设定下$k$-局域哈密顿量问题的半定规划与非交换平方和分层在有限层级上的显式收敛速率：对于仅含偶权重项的$k$-局域哈密顿量，NPA型下界分层和谱最小上界分层的误差至多为$C(k)\xi^{n,4}_{d+1}/n$，其中$\xi^{n,4}_{d+1}$是Krawtchouk多项式的最小根；一般$k$-局域哈密顿量可通过添加一个辅助量子比特归约到该情形。证明通过构造Pauli代数上的几乎再生核并将其谱与Krawtchouk多项式关联，给出了非交换半定松弛保里哈密顿量的首个定量有限层级精度保证。


<details>
  <summary>Details</summary>
Motivation: $k$-局域哈密顿量问题是量子多体系统与哈密顿量复杂性的核心模型。尽管半定规划和非交换平方和分层能为基态能量提供系统性认证，但现有的有限收敛结果无法给出计算中可触及的低层级精度的定量保证。本文旨在填补这一空白，建立这些分层在Pauli设定下的显式有限层级收敛速率。

Method: 针对Pauli展开仅含偶权重项（每个Pauli算子的权重为偶数）的$k$-局域哈密顿量，分析NPA型下界分层和上界分层在谱最小值上的误差。证明构造了Pauli代数上的几乎再生核（almost-reproducing kernels），将其谱关联到Krawtchouk多项式，从而将问题转化为交换多项式优化的核方法分析框架的非交换类比。一般$k$-局域哈密顿量通过添加一个辅助量子比特（ancilla qubit）保持谱的同时归约到偶权重情形。

Result: 对于偶权重$k$-局域哈密顿量，NPA型下界分层和上界分层的误差上界均为$C(k)\xi^{n,4}_{d+1}/n$，其中$\xi^{n,4}_{d+1}$是Krawtchouk多项式的最小根，$C(k)$与量子比特数$n$和分层层级$d$无关。这是首个非交换半定松弛（Pauli哈密顿量）的定量有限层级精度保证。

Conclusion: 该工作为Pauli哈密顿量的非交换半定松弛提供了首个定量有限层级收敛速率，连接了非交换多项式优化与交换情形下基于核方法的收敛分析，为量子多体系统基态能量的可计算认证建立了理论保障。
...
TL;DR: 本文提出了一种零错误换能器来实现电通量采样（elfs），并建立了关于子空间交的零错误反射换能器，从而改进了有效电阻估计、span program见证大小估计和随机游走到达分布采样等量子游走算法，并在扩展图上的半监督学习中获得最高二次量子加速。


<details>
  <summary>Details</summary>
Motivation: 电通量采样（elfs）是量子游走工具箱中的新工具，可用于图上的搜索、采样和优化问题。作者希望进一步精炼这一工具，实现零错误版本，从而提升相关量子算法的性能。

Method: 证明存在零错误换能器实现elfs；建立关于两个子空间交的零错误反射换能器，推导出无错误的换能器版有效间隙引理；在此基础上设计改进的量子游走算法。

Result: 获得了有效电阻和span program见证大小估计的最优误差标度量子算法；通过多个elfs的复合实现了随机游走到达分布采样的改进算法；在扩展图上的半监督学习中获得了最高二次量子加速。

Conclusion: 零错误elfs换能器的存在不仅提升了原有量子游走算法的精度，还扩展了其应用范围，特别是在半监督学习等任务中展现了显著的量子优势。
...
TL;DR: 本文提出一种几何机制解释非单比特噪声如何加速量子态向Haar随机分布的趋近。通过振幅阻尼信道与重正化规则结合，在纯态流形上定义非线性扩张映射，推导出局域面积扩张因子与全局扩张阈值。结合通用门集G3={H,T,CNOT}的实验表明，在适当参数区间内噪声确实能加速Haar-like行为趋近，提出噪声可作为量子算法资源的观点。


<details>
  <summary>Details</summary>
Motivation: 量子噪声通常被视为实现可扩展量子优势的主要障碍，但近期量子储层计算的研究表明某些噪声通道可在适当条件下提升性能。受此启发，本文旨在探索噪声是否以及如何作为一种资源加速量子算法，并建立其几何机制解释。

Method: 采用最小1量子比特模型，将振幅阻尼信道与重正化规则（将混合态关联到代表性纯态）结合，在纯态空间上定义全局扩张的非线性映射。解析推导局域面积扩张因子，识别全局扩张阈值。最后将振幅阻尼与通用门集G3={H,T,CNOT}结合，数值验证参数区间内的加速效应。

Result: 发现非单比特噪声在纯态流形上诱导有效体积扩张；解析得出局域面积扩张因子及全局扩张阈值；实验表明在适当参数区间内，噪声结合通用门集可加速向Haar-like分布的趋近。

Conclusion: 提出噪声在特定条件下可作为量子算法的潜在资源，挑战了传统"噪声完全有害"的观念，为利用噪声设计更高效的量子算法提供了理论依据。
...
TL;DR: 本文通过微观Bloch-Redfield方法重新审视了耗散驱动的量子比特-谐振子系统动力学，发现其与常用的Lindblad主方程在无驱动时存在定量差异，在驱动时存在定性差异，并研究了Purcell滤波器对测量诱导弛豫的抑制效应。


<details>
  <summary>Details</summary>
Motivation: 常用的Lindblad主方程使用子系统局域跳跃算子来建模耗散驱动的量子比特-谐振子系统，即使在量子比特和谐振子显著杂化的情况下也是如此。本文旨在通过更微观的Bloch-Redfield方法重新审视这一设定，以揭示Lindblad近似的局限性。

Method: 采用微观Bloch-Redfield方法，在耦合量子比特-谐振子哈密顿量的本征态基下构建耗散，并利用传输线环境的完整频率依赖开系统描述。

Result: 在无驱动情况下，Lindblad和Bloch-Redfield衰减率存在定量差异；在驱动情况下，时间无关的Redfield耗散子与其时间依赖推广形式表现出定性不同的行为；在结构化光谱密度中，Purcell滤波器能抑制测量诱导的弛豫。

Conclusion: 表明在量子比特-谐振子杂化系统中，使用Lindblad主方程可能不够准确，特别是当驱动存在时，需要采用更完整的Bloch-Redfield方法。同时验证了Purcell滤波器在抑制测量诱导弛豫方面的有效性。
...
TL;DR: 本书探讨了量子力学中若干基础性问题，包括微观世界概率起源、普朗克常数h的本质、概率振幅的性质、波函数的含义，并建立了量子理论公理体系，研究了将热浴中经典振子解释为量子振子的模型，详细讨论了测量问题。


<details>
  <summary>Details</summary>
Motivation: 作者旨在澄清量子力学中通常不被讨论的基础性问题，如概率起源、普朗克常数本质、概率振幅和波函数的性质，试图从更根本的层面理解量子理论。

Method: 通过建立量子理论公理体系，以及研究经典振子在热浴中的模型（将经典系统解释为量子系统）来探讨基础问题。

Result: 形成了量子理论的公理化表述，提出了一种将经典振子解释为量子振子的模型，并对测量问题进行了详细讨论。

Conclusion: 该书适用于对量子理论基础感兴趣的高年级本科生、研究生及专家，提供了对量子力学基础问题的新视角和分析。
...
TL;DR: 本文发现魔法态蒸馏协议的开销指数与渐近蒸馏率之间的定量关系并不稳健，提出了一种开销指数大于1但仍能实现接近线性渐近蒸馏率的协议，表明在次线性速率区域，蒸馏率不受开销指数的约束。


<details>
  <summary>Details</summary>
Motivation: 先前研究中，开销指数（描述目标蒸馏误差下噪声魔法态数量的标度）和渐近蒸馏率（输出与输入魔法态的最大比例）在特定情形（零开销指数与线性渐近率等价）下紧密相关，但在其他区域的关系尚不明确。本文旨在探究两者在更一般情况下的定量关系。

Method: 提出了一族基于逻辑Clifford算子测量进行错误检查的魔法态蒸馏协议（类似于近期魔法态培育协议），通过分析该协议的开销指数和渐近蒸馏率，揭示两者在次线性速率区域存在脱耦关系。

Result: 该协议的开销指数大于1（非接近零），但渐近蒸馏率可以任意接近线性速率（即输出/输入魔法态比例趋近于1）。这表明开销指数与渐近蒸馏率在次线性速率区域不存在约束关系。

Conclusion: 魔法态蒸馏的速率不受开销指数在次线性速率区域内的约束，基于逻辑Clifford算子测量错误检查的机制在渐近魔法态蒸馏中具有潜力。
...
TL;DR: 本文提出了一种通用框架，在基于门电路的量子计算机上实现“绝热量子计算”的替代过程，避免模拟含时哈密顿量演化的开销。应用于量子线性系统问题（QLSP），实现了条件数的最优缩放，并展示了Trotter化在随机化场景下的保真度误差界优于标准结果。


<details>
  <summary>Details</summary>
Motivation: 传统绝热量子计算依赖含时哈密顿量演化，在门电路量子计算机上实现时需额外模拟开销。作者希望找到不依赖含时演化、易于在门电路量子计算上实现的替代方案，并给出通用的理论框架。

Method: 提出多种替代绝热过程的算法框架，推导适用于这些过程的“绝热定理”。应用包括：求解QLSP（条件数最优缩放）、随机化版本的离散绝热算法（Costa et al. 2022），以及随机化Trotter化框架，从保真度角度获得渐近更优的Trotter误差界。

Result: 给出了QLSP的最优条件数缩放的算法；复现并改进了Costa等人和An等人的结果；在随机化设置下得到了渐近优于标准界的Trotter误差保真度界。

Conclusion: 提出的框架为在门电路量子计算机上高效实现类绝热过程提供了通用工具，尤其在线性系统求解和Trotter误差分析中展现了理论优势。
...
TL;DR: 本文扩展了多相互作用世界方法，将其应用于具有奇异性势（如库仑势和有限陷阱势）的一维和二维有界系统，通过渐近平滑方法处理奇点，数值模拟结果与标准量子力学中的矩阵Numerov方法一致。


<details>
  <summary>Details</summary>
Motivation: 2014年提出的确定性多相互作用世界方法在谐振子势中已成功应用，但尚未充分验证于其他有界系统。本文旨在将该方法推广到具有奇异势（库仑势、有限陷阱势）的一维和二维系统，探索其更广泛的适用性。

Method: 采用多相互作用世界方法中的动力学算法，通过渐近平滑方法处理库仑势和有限陷阱势中的奇异性，在二维情形下进行扩展，数值模拟逼近给定系统的定态。

Result: 数值模拟结果表明，该方法在求解有界系统时与标准量子力学中的矩阵Numerov方法具有一致性。

Conclusion: 多相互作用世界方法可成功应用于具有奇异势的有界系统，为求解更复杂系统提供了可能。
...
TL;DR: 提出三阶段量子基态制备框架：DMRG生成MPS → 优化自由MPD编码电路加载到量子寄存器 → PITE抑制残余误差。MPD编码中间态中心键Schmidt秩随层数逻辑增长，拐点L*标记高效编码边界；超出后保真度增益放缓，额外层数标度为O(N^5 log(N/ε))。数值实验表明该框架避免过深编码电路并抑制PITE后选择开销。


<details>
  <summary>Details</summary>
Motivation: 基态制备是量子模拟的基础任务，制备态与真实基态的重叠度显著影响后续量子算法的整体成本。现有方法面临编码电路过深或后选择开销过大的问题，需要一种高效混合策略。

Method: 三阶段框架：(1) DMRG计算N-比特系统的MPS；(2) 通过优化自由的MPD编码电路将MPS加载到量子寄存器，分析中间态Schmidt秩的逻辑增长及拐点L*；(3) 在L*处停止编码，使用PITE抑制残余激发态成分，PITE线性调度由基态能量、有效能隙和DMRG估计的参考重叠固定。

Result: MPD编码中间态中心键Schmidt秩随层数逻辑增长，L*后保真度增益迅速放缓；L*后达到目标保真度所需额外层数标度为O(N^5 log(N/ε))。数值实验显示框架避免过深编码电路，并显著抑制PITE后选择开销。

Conclusion: 结合DMRG经典预处理、优化自由MPS编码和确定性调度PITE，该混合框架为量子模拟中的基态制备提供了一条实用途径。
...
TL;DR: 本文提出了将经典分子动力学中的Green-Kubo输运系数计算转化为量子算法可求解的问题，利用Koopman-von Neumann表示将NVE和NVT动力学构造为希尔伯特空间上的幺正演化。数值基准表明离散化误差随网格点数呈幂律下降，等效于随量子比特数指数下降。通过量子相位估计读取输运系数，并结合最大似然振幅估计将统计估计误差从N^{-1/2}提升至接近N^{-1}。电路资源分析显示NVE传播子需要O(n²)个CX门，而NVT传播子的Nosé-Hoover摩擦项需要O(n_ξ n_p 2^{n_p})个CX门。


<details>
  <summary>Details</summary>
Motivation: 将经典分子动力学中的Green-Kubo输运系数计算转化为适合量子算法求解的问题，以期利用量子计算的加速优势来提高实际分子模拟中输运系数计算的效率。

Method: 利用Koopman-von Neumann表示将NVE和Nosé-Hoover型NVT经典动力学构造为希尔伯特空间上的幺正演化；在有限网格上数值验证离散化误差；通过量子相位估计(QPE)读取输运系数，并用最大似然振幅估计提升统计效率；进行电路资源分析。

Result: 离散化误差随网格点数N_z呈幂律下降，随量子比特数n_z指数下降，目标精度ε需要n_z=O(log(1/ε))量子比特。最大似然振幅估计将统计误差从N^{-1/2}提升至接近N^{-1}。NVE传播子需O(n²)个CX门，NVT传播子的Nosé-Hoover摩擦项需O(n_ξ n_p 2^{n_p})个CX门。

Conclusion: 该框架是将量子算法原理应用于实际分子模拟中输运系数计算的具体步骤，为量子计算在经典分子动力学模拟中的应用提供了可行途径。
...
TL;DR: 本文构建了一个用于正则系综（NVT）有限温度分子动力学的量子电路框架，通过Langevin恒温器实现经典核相空间的正则态制备与物理可观测量读取。将经典相空间分布编码为Koopman-von Neumann波函数，利用Langevin型Fokker-Planck弛豫制备正则态，并将哈密顿刘维尔流、动量摩擦与动量扩散分解为独立的电路模块。摩擦模块采用对称化的动量空间扩张，扩散模块通过概率虚时演化（PITE）实现的余弦滤波器来完成。理论分析量化了PITE余弦滤波器替代高斯扩散核引起的温度偏差，并给出内部温度修正以恢复目标平衡分布。以H₂分子为验证系统，数值模拟展示了从非平衡相空间分布到正则KvN态的弛豫过程，实现了振动态密度和过渡态理论速率常数的读取。该工作为量子-经典混合分子动力学在量子计算机上的实现提供了具体的电路级协议。


<details>
  <summary>Details</summary>
Motivation: 将量子计算与经典分子动力学结合，实现正则系综下有限温度分子动力学的量子电路模拟，为量子-经典混合分子动力学在量子计算机上的应用提供可行的电路级协议。

Method: 采用Koopman-von Neumann（KvN）量子化框架将经典相空间分布编码为波函数；将Langevin动力学分解为刘维尔流、动量摩擦（对称化动量空间扩张）和动量扩散（PITE实现的余弦滤波器）三个电路模块；通过理论分析推导PITE余弦滤波器与高斯扩散核之间的温度偏差并给出修正方案；以H₂分子为例进行数值验证。

Result: 数值模拟证明了从非平衡分布到正则KvN态的弛豫过程；从制备的正则态中成功读取了H-H伸缩振动的振动态密度（通过动态量子相位估计）和过渡态理论速率常数。

Conclusion: 该工作在一个最小分子系统（H₂）上完整展示了连接Langevin正则态制备到物理性质计算的电路级协议，为未来在量子计算机上实现量子-经典混合分子动力学提供了具体可行的步骤和理论基础。
...
TL;DR: 本文提出一种机器学习框架——隧穿相图，用于从动力学同位素效应（KIE）中解耦出真实的量子隧穿强度κ，解决了传统KIE因包含零点能和经典动力学而混淆隧穿效应的问题。该框架在R²>0.98、RMSE=0.21的高精度下，发现了300-600K区间内高KIE-低κ的异常区域，为量子隧穿的定量评估提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 传统KIE作为量子隧穿的探针具有复合性，同时包含了零点能效应、经典动力学和真实隧穿贡献，无法直接反映隧穿强度。需要一种方法将隧穿效应从KIE中解耦出来。

Method: 提出隧穿相图，利用机器学习框架解码KIE与隧穿因子κ之间的非线性关系，从而实现隧穿强度的解耦。

Result: 该框架具有卓越的预测精度（R²>0.98, RMSE=0.21），并发现了300-600K温度范围内存在的高KIE-低κ的异常区域。

Conclusion: 该工作为量子隧穿的定量评估建立了新范式，揭示了传统KIE指标在表征隧穿效应时的局限性。
...
TL;DR: 本文提出将逻辑噪声转化为可控资源，通过部分量子纠错将容错结构重新用于塑造耗散，实现开放量子系统的高效量子模拟。


<details>
  <summary>Details</summary>
Motivation: 在开放量子动力学中，耗散是目标物理的一部分，但传统容错架构旨在抑制退相干，二者存在张力。需要一种方法将噪声转化为可利用的校准资源。

Method: 将纠错周期视为可编程原语：一轮容错纠错诱导逻辑完全正迹保持映射，通过解码器/恢复随机化生成可控逻辑通道族，其凸混合实现Kraus通道混合。选择码距使未受控逻辑误差仅占每步预期耗散的小部分。

Result: 实现了将目标耗散器直接编译为有效逻辑动力学，无需额外辅助量子比特编码浴自由度。推导出多步模拟的精度判据。
...
TL;DR: 本文展示了明亮压缩真空光结合单次正交测量，能够在弱耦合条件下通过正交基预报方法，在物质中超快制备高斯权重量子叠加态（如Dicke态和类猫态），为利用强场量子光超快调控宏观量子物质开辟新路径。


<details>
  <summary>Details</summary>
Motivation: 利用明亮压缩真空光与单次正交测量，实现物质中宏观量子态的超快制备，克服弱耦合条件下光-物质纠缠导致物质态仅为经典混合物的限制。

Method: 采用明亮压缩真空光激发共振电偶极耦合的两能级系统，通过单次正交测量后选择（heralding）机制，实现对电极化的高斯滤波；结合反旋项驱动，实现从Dicke态到类猫态的随机跃迁。

Result: 正交基预报动力学等效于对电极化的高斯滤波；更亮的压缩真空光加速零本征值Dicke态的制备；反旋项驱动使Dicke态随机转变为类猫态。

Conclusion: 该研究为利用强场量子光实现宏观量子物质的超快工程化提供了新途径。
...
TL;DR: 本文提出了一种面向混合振荡器-量子比特量子处理器的硬件原生非阿贝尔混合器，用于QAOA算法，并在Max-Cut问题上展示了相比标准横向场混合器更优的近似比和最优解采样概率。


<details>
  <summary>Details</summary>
Motivation: 混合振荡器-量子比特量子处理器（CV-DV混合系统）能够实现通用量子控制，但其算法开发仍处于早期阶段。QAOA在纯CV和纯DV系统中已被广泛研究，但在混合CV-DV场景下的发展十分有限。

Method: 提出了一种硬件原生的非阿贝尔混合器，并基于此开发了针对Max-Cut问题的混合变分拟设（hybrid ansatz）。在无权重Erdős-Rényi图上进行仿真，与标准横向场混合器进行对比。

Result: 在所有图规模和Fock截断下，所提出的非阿贝尔混合器在近似比和最优解采样概率两方面均一致优于横向场混合器。

Conclusion: 该非阿贝尔混合器是面向混合振荡器-量子比特平台上QAOA算法的一个有前景的基础构建模块。
...
TL;DR: 本文研究对称性约束对量子过程中因果关系不定性的影响，发现有限酉对称性下不产生新关联，但实量子理论在不定因果序下能实现比复量子理论更丰富的过程关联。


<details>
  <summary>Details</summary>
Motivation: 探究因果关系能否像其他物理性质一样具有量子不定性，并分析对称性约束如何影响这种不定性所允许的量子过程。

Method: 采用过程矩阵框架，在局部实验室受对称性（如缺失参考系、超选择约束、反酉对称性）限制的条件下，分析可实现的量子过程锥的变化。

Result: 有限酉对称性约束下不会产生新的过程关联（尖锐二分性结论）；而实量子理论在不定因果序下能实现比复量子理论更多的过程关联。

Conclusion: 在确定因果序下复量子理论更丰富，但在不定因果序下这一层级关系被逆转：实量子理论反而更丰富。
...
TL;DR: 本文研究了高阶无约束二元优化（HUBO）公式在工业调度和运输路径问题中映射到量子优化工作流的应用，对比了HUBO与标准QUBO编码的权衡：HUBO减少量子比特需求但增加电路深度。通过经典求解器和小规模路由问题的偏置场数字化反绝热量子优化仿真验证，并基于容量车辆路径问题进行资源和可扩展性分析，指出混合量子-经典工作流和早期容错量子硬件是实际应用的最可行场景。


<details>
  <summary>Details</summary>
Motivation: 工业调度和运输路径问题的复杂性日益增加，而标准二次无约束二元优化（QUBO）形式难以忠实表达高度相关的装配线调度规则等过程复杂性，且量子映射中需要大量二元变量。因此，研究高阶无约束二元优化（HUBO）公式能否更有效地映射到量子优化工作流是本文的核心动机。

Method: 选取三个代表性物流和制造用例，将其分别建模为HUBO问题；与对应QUBO编码进行对比分析；使用经典求解器在多个问题实例上验证；对小规模路由问题实例采用偏压场数字化反绝热量子优化进行经典仿真；以容量车辆路径问题为代表进行资源和可扩展性分析。

Result: HUBO公式相较于QUBO在量子比特缩放方面具有优势，能通过紧凑二元编码减少量子比特需求。但HUBO引入的高阶交互项增加了电路深度，受限于当前量子硬件的门保真度、相干性和电路深度。偏压场数字化反绝热量子优化在小规模基准问题上得到验证。

Conclusion: HUBO公式在实际应用中的可行性受限于当前量子硬件的局限性，混合量子-经典工作流和早期容错量子硬件是其最可能的实际应用场景。量子比特需求和电路深度之间存在关键权衡：HUBO降低量子比特需求但增加电路深度。
...
TL;DR: 本文展示了玻色子模式在量子同步中能达到具有负Wigner函数的Fock态极限环稳态，并通过外部驱动实现相位锁定（Arnold舌区域），证明同步是依赖相位滑移指数级抑制的动力学特性，并提出了从Lindblad时间演化中提取相位滑移率的新方法。


<details>
  <summary>Details</summary>
Motivation: 将经典同步现象拓展到量子领域，探索玻色子模式能否实现具有非经典特性的量子同步，以及同步与相位滑移之间的内在动力学联系。

Method: 通过Fock态极限环实现稳态负Wigner函数，施加外部驱动实现Arnold舌区域内的相位锁定；利用Lindblad主方程时间演化提取相位滑移率，分析相位滑移概率的指数衰减行为。

Result: 玻色子模式成功实现了非经典（负Wigner函数）稳态同步，同步发生在Arnold舌区域内，相位滑移概率呈指数衰减。

Conclusion: 量子同步本质上是由相位滑移的指数级抑制所驱动的动力学过程，该工作为理解和操控非经典同步动力学开辟了新途径。
...
TL;DR: 本文针对基于Wave Matrix Lindbladization (WML)算法的Lindbladian模拟，建立了改进的样本复杂度界。显式非渐近界为 n_d^*(t,ε) ≤ ((2d+3)/8) ||L||_∞^2 (t^2/ε)，改进了此前O(d^2 t^2/ε)的结果。当||L||_∞^2 = O(1/d)时（随机Lindblad算子高概率满足），样本复杂度可降至O(t^2/ε)；但在最坏情况下，通过秩一Lindblad算子构造，证明WML需要Ω(d t^2/ε)样本。揭示了典型与对抗样本复杂度之间的尖锐二分性。


<details>
  <summary>Details</summary>
Motivation: 现有的样本复杂度界存在O(d^2)维度过高开销，且缺乏对典型情形与最坏情形的明确区分。本文旨在精确刻画Lindbladian模拟的样本复杂度，建立更紧的界并揭示其在不同情形下的本质差异。

Method: 理论分析：推导WML算法的显式非渐近样本复杂度界；构造秩一Lindblad算子作为最坏情况反例以证明下界；分析随机Lindblad算子下||L||_∞^2 = O(1/d)的高概率满足条件。

Result: 改进了样本复杂度界从O(d^2 t^2/ε)到O(d t^2/ε)（最坏情况），并在典型情形下达到O(t^2/ε)（与d无关）；建立了Ω(d t^2/ε)的最坏情况匹配下界。

Conclusion: 揭示了Lindbladian模拟中样本复杂度在典型情形与最坏情形之间的尖锐二分性：典型情形下可实现与维度无关的样本复杂度，而对抗情形下线性维度依赖不可避免，这为样本基量子算法提供了更坚实的理论基础。
...
TL;DR: 本文利用Keldysh路径积分方法研究连续变量量子系统的同步现象，发现量子相位滑移会破坏极限环的相位锁定，类似于经典同步中噪声引起的相位滑移，并分析了非马尔可夫效应对同步的影响。


<details>
  <summary>Details</summary>
Motivation: 经典物理中弱耦合自持振荡器可自发锁相，但量子系统中同步如何被破坏尚不明确。本文旨在揭示量子相位滑移如何导致连续变量量子系统同步的失效。

Method: 采用Keldysh路径积分形式对极限环进行建模，分析相位动力学；并通过电压偏置双量子点耦合的超导谐振腔系统，具体说明非马尔可夫效应对同步的影响。

Result: 发现量子相位滑移的增殖会破坏实际的相位锁定，即便存在强相位关联；非马尔可夫效应也会影响极限环的同步行为。

Conclusion: 量子相位滑移是破坏量子系统同步的关键机制，Keldysh路径积分方法为研究非马尔可夫环境下的量子同步提供了有效框架。
...
TL;DR: 本文提出了一个基于分步法和模式光场表示的解析框架，用于描述空间光模式在湍流大气中的传播。该框架揭示了湍流引起的相位波动如何将光功率从原始模式重新分配到相邻空间模式，且功率转移随传播距离线性缩放，可通过矩阵指数形式求解任意距离下的平均功率分布。转移速率由湍流谱与模式间接受谱的空间谱重叠决定。模型在单一模式占主导时精确，与中强湍流下的仿真结果吻合良好，并验证了此前经验观察到的模式阶数缩放规律。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如数值仿真）计算复杂，难以直观理解湍流对空间模式耦合的物理机制。作者希望建立一个简洁、可解析求解的框架，直接量化湍流诱导的模式间功率转移，并揭示其与湍流谱和模式特性的关系。

Method: 基于分步法（split-step approach），将光场表示为空间模式的叠加；分析湍流引起的相位扰动如何导致模式间耦合；推导出功率转移速率正比于湍流谱与模式接受谱（acceptance spectrum）的空间谱重叠；通过矩阵指数形式得到任意距离下的平均模式功率分布。

Result: 模型预测与中强湍流下的数值仿真结果有合理的一致性；验证了模式阶数与湍流效应缩放关系的经验观察；在单一模式主导时模型精确成立。

Conclusion: 该解析框架为理解湍流对空间光模式传播的影响提供了简洁、可扩展的工具，适用于自由空间光通信、光学成像等涉及空间模式的应用场景。
